//! Encoder module for CMP instruction forms.
//! This is built from https://www.felixcloutier.com/x86/cmp
...
const CMP_OPCODE = struct {
    const CMP_RM8_R8: u8 = 0x38;
    const CMP_RM16_R16: u8 = 0x39;
    const CMP_RM32_R32: u8 = 0x39;
    const CMP_RM64_R64: u8 = 0x39;

    const CMP_R8_RM8: u8 = 0x3A;
    const CMP_R16_RM16: u8 = 0x3B;
    const CMP_R32_RM32: u8 = 0x3B;
    const CMP_R64_RM64: u8 = 0x3B;

    const CMP_RM8_IMM8: u8 = 0x80; // /7
    const CMP_RM16_IMM16: u8 = 0x81; // /7
    const CMP_RM32_IMM32: u8 = 0x81; // /7
    const CMP_RM64_IMM32: u8 = 0x81; // /7 + REX.W
};

const Writer = std.Io.Writer;

pub const rm8_r8 = factory_op(RegisterMemory8, Register8, CMP_OPCODE.CMP_RM8_R8);
pub const r8_rm8 = factory_op(Register8, RegisterMemory8, CMP_OPCODE.CMP_R8_RM8);

pub const rm16_r16 = factory_op(RegisterMemory16, Register16, CMP_OPCODE.CMP_RM16_R16);
pub const r16_rm16 = factory_op(Register16, RegisterMemory16, CMP_OPCODE.CMP_R16_RM16);

pub const rm32_r32 = factory_op(RegisterMemory32, Register32, CMP_OPCODE.CMP_RM32_R32);
pub const r32_rm32 = factory_op(Register32, RegisterMemory32, CMP_OPCODE.CMP_R32_RM32);

pub const rm64_r64 = factory_op(RegisterMemory64, Register64, CMP_OPCODE.CMP_RM64_R64);
pub const r64_rm64 = factory_op(Register64, RegisterMemory64, CMP_OPCODE.CMP_R64_RM64);

pub const rm8_imm8 = factory_imm(
    RegisterMemory8,
    u8,
    .{ .mode = .modrm_group, .opcode = CMP_OPCODE.CMP_RM8_IMM8, .modrm_reg = 0b111 },
);
pub const rm16_imm16 = factory_imm(
    RegisterMemory16,
    u16,
    .{ .mode = .modrm_group, .opcode = CMP_OPCODE.CMP_RM16_IMM16, .modrm_reg = 0b111 },
);
pub const rm32_imm32 = factory_imm(
    RegisterMemory32,
    u32,
    .{ .mode = .modrm_group, .opcode = CMP_OPCODE.CMP_RM32_IMM32, .modrm_reg = 0b111 },
);
pub const rm64_imm32 = factory_imm(
    RegisterMemory64,
    u32,
    .{ .mode = .modrm_group, .opcode = CMP_OPCODE.CMP_RM64_IMM32, .modrm_reg = 0b111 },
);
