...
pub fn append(self: *Engine, id: SegmentId, bytes: []const u8) !u64 {
    ...
    const offset = segmentFileSize(state);
...
/// Reserves zero-initialized memory that is present at load time but omitted from the file image.
pub fn reserveBss(self: *Engine, id: SegmentId, len: usize) !u64 {
    const state = try self.segmentState(id);
    const offset = segmentMemorySize(state);
    state.bss_size = std.math.add(u64, state.bss_size, len) catch return error.Overflow;
    return offset;
}
...
pub fn setEntry(self: *Engine, segment_id: SegmentId, offset: u64) !void {
    ...
    if (offset > segmentFileSize(state)) return error.InvalidEntryOffset;
...
pub fn payloadSlice(self: *Engine, segment_id: SegmentId, offset: u64) ![]u8 {
    const state = try self.segmentState(segment_id);
    if (offset > segmentFileSize(state)) return error.InvalidSegmentOffset;
...
pub fn payloadVirtualAddress(self: *Engine, segment_id: SegmentId, offset: u64) !u64 {
    const state = try self.segmentState(segment_id);
    if (offset > segmentMemorySize(state)) return error.InvalidSegmentOffset;
...
fn ensurePatchRange(self: *Engine, segment_id: SegmentId, offset: u64, size: u64) !void {
    ...
    if (end > segmentFileSize(state)) return error.InvalidPatchOffset;
}

fn resolveEntry(self: *Engine, resolved: []const ResolvedSegment, entry: EntryPoint) !u64 {
    const layout = resolved[entry.segment.index];
    const segment_size = segmentFileSize(&self.segments.items[entry.segment.index]);
...
fn resolveLayout(self: *Engine) ![]ResolvedSegment {
    ...
    for (self.segments.items, 0..) |state, i| {
        if (segmentMemorySize(&state) == 0) return error.EmptySegment;

        const payload_file_len = segmentFileSize(&state);
        const payload_mem_len = segmentMemorySize(&state);
        ...
        if (i == 0) {
            resolved[i] = .{
                ...
                .file_size = header_size + payload_file_len,
                .mem_size = header_size + payload_mem_len,
                .alignment = alignment,
                .flags = flags,
            };
            cursor = header_size + payload_file_len;
            next_load_vaddr = self.image_base + header_size + payload_mem_len;
            continue;
        }
        ...
        resolved[i] = .{
            ...
            .file_size = payload_file_len,
            .mem_size = payload_mem_len,
            .alignment = alignment,
            .flags = flags,
        };
        cursor += payload_file_len;
        next_load_vaddr = load_vaddr + payload_mem_len;
    }
...
fn segmentFileSize(sgmt: *const SegmentState) u64 {
    return @intCast(sgmt.bytes.items.len);
}

fn segmentMemorySize(sgmt: *const SegmentState) u64 {
    return segmentFileSize(sgmt) + sgmt.bss_size;
}

fn totalFileSize(resolved: []const ResolvedSegment) usize {
    var size: u64 = 0;
    for (resolved) |sgmt| {
        size = @max(size, sgmt.load_offset + sgmt.file_size);
    }
    return @intCast(size);
}
