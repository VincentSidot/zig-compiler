//! Simple example of executing jit compiled fibonacci function.
...
fn generate_code_engine(allocator: std.mem.Allocator) ![]u8 {
...
pub fn run(mode: Mode) !void {
    ...
    const bytecode = switch (mode) {
        .Legacy => try generate_code_legacy(allocator),
        .Engine => try generate_code_engine(allocator),
    };
